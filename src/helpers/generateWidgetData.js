  ...
  if (!chosenGoal && !chosenCompanyType) {
    ...
    data.forEach(d => {
      if (d.companyType === chosenCompanyType && d.goals[chosenGoal]) {
        if (d.goals[chosenGoal].maxLoan > maxLoan) maxLoan = d.goals[chosenGoal].maxLoan;
        if (d.goals[chosenGoal].maxTimeInMonths > maxTime) maxTime = d.goals[chosenGoal].maxTimeInMonths
      }
    })
  }
  return {
